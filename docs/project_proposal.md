...
erDiagram
    ...
    CVE_VULNERABILITIES ||--o| CISA_KNOWN_EXPLOITED_VULNERABILITIES : "동일 CVE 원본 보강"
    ...
    CISA_KNOWN_EXPLOITED_VULNERABILITIES {
        TEXT cve_id PK "CISA KEV 원본 CVE ID"
        TEXT vendor_project "공급업체"
        TEXT product "제품명"
        TEXT vulnerability_name "취약점명"
        DATE date_added "KEV 등록일"
        TEXT short_description "원본 취약점 설명"
        TEXT required_action "CISA 권고 조치"
        DATE due_date "대응 마감일"
        TEXT known_ransomware_campaign_use "랜섬웨어 활용 여부"
        TEXT notes "참고 URL"
        TEXT cwes "CWE 분류 코드"
        TEXT source_file "원본 파일명"
        TIMESTAMPTZ imported_at "적재 시각"
    }
...
COMMENT ON COLUMN cisa_known_exploited_vulnerabilities.cve_id                         IS 'CISA KEV 원본 카탈로그의 CVE 고유 식별자';
COMMENT ON COLUMN cisa_known_exploited_vulnerabilities.vendor_project                 IS '취약점이 존재하는 공급업체 또는 프로젝트명';
COMMENT ON COLUMN cisa_known_exploited_vulnerabilities.product                        IS '취약한 제품 또는 서비스명';
COMMENT ON COLUMN cisa_known_exploited_vulnerabilities.vulnerability_name             IS 'CISA 원본 카탈로그의 취약점 명칭';
COMMENT ON COLUMN cisa_known_exploited_vulnerabilities.date_added                     IS 'CISA KEV 카탈로그 등록일';
COMMENT ON COLUMN cisa_known_exploited_vulnerabilities.short_description              IS 'CISA 원본 취약점 요약 설명';
COMMENT ON COLUMN cisa_known_exploited_vulnerabilities.required_action                IS 'CISA가 권고한 대응 조치';
COMMENT ON COLUMN cisa_known_exploited_vulnerabilities.due_date                       IS '대응 마감일';
COMMENT ON COLUMN cisa_known_exploited_vulnerabilities.known_ransomware_campaign_use  IS '랜섬웨어 캠페인 활용 여부(Known/Unknown)';
COMMENT ON COLUMN cisa_known_exploited_vulnerabilities.notes                          IS 'CISA, 벤더, NVD 등 참고 URL';
COMMENT ON COLUMN cisa_known_exploited_vulnerabilities.cwes                           IS 'CWE 분류 코드';
COMMENT ON COLUMN cisa_known_exploited_vulnerabilities.source_file                    IS '적재에 사용한 원본 CSV 파일명';
COMMENT ON COLUMN cisa_known_exploited_vulnerabilities.imported_at                    IS 'Neon DB 적재 시각';

-- 4) OTX-CVE 연결 테이블 (FK 관계 명시)
...
> **FK 설계 보완**: 하나의 Neon database(`threat_intel_agent`) 안에 OTX, CVE, CISA KEV 원본, 악성 도메인, 악성 IP, 연결 테이블을 함께 구성했습니다. 원천 CSV의 OTX 보고서, CVE, 도메인, IP는 서로 다른 공개 인텔리전스 소스에서 수집되어 직접적인 ID 기반 FK가 부족합니다. 따라서 루브릭의 PK/FK 요건과 실제 분석 활용성을 함께 만족시키기 위해 `threat_intel_links` 연결 테이블을 추가하고, OTX 보고서(`pulse_id`)와 CVE(`cve_id`) 사이의 키워드 기반 연관 관계를 FK로 명시했습니다. `cisa_known_exploited_vulnerabilities`는 CISA KEV 원본 CSV를 보존하는 테이블이며, `cve_vulnerabilities.cve_id`와 동일한 CVE 식별자를 통해 논리적으로 연결됩니다. 도메인/IP 테이블은 IOC 평판 속성 테이블로 유지하고, OTX와의 연관성은 자연어 질의 및 키워드/ATT&CK 기반 분석에서 활용합니다.
...
### 4.3 `cisa_known_exploited_vulnerabilities` (10건)

| cve_id | vendor_project | product | vulnerability_name | date_added | due_date | known_ransomware_campaign_use | notes | cwes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CVE-2026-31431 | Linux | Kernel | Linux Kernel Incorrect Resource Transfer B... | 2026-05-01 | 2026-05-15 | Unknown | vendor, xint, kernel, NVD URLs | CWE-699 |
| CVE-2026-41940 | WebPros | cPanel & WHM and WP2 (WordPress Squared) | WebPros cPanel & WHM and WP2 Missing Auth... | 2026-04-30 | 2026-05-03 | Unknown | cPanel, WP2, NVD URLs | CWE-306 |
| CVE-2024-1708 | ConnectWise | ScreenConnect | ConnectWise ScreenConnect Path Traversal V... | 2026-04-28 | 2026-05-12 | Unknown | ConnectWise advisory, NVD URL | CWE-22 |
| CVE-2026-32202 | Microsoft | Windows | Microsoft Windows Protection Mechanism Fai... | 2026-04-28 | 2026-05-12 | Unknown | MSRC, NVD URL | CWE-693 |
| CVE-2025-29635 | D-Link | DIR-823X | D-Link DIR-823X Command Injection Vulnerab... | 2026-04-24 | 2026-05-08 | Unknown | D-Link advisory, NVD URL | CWE-77 |
| CVE-2024-7399 | Samsung | MagicINFO 9 Server | Samsung MagicINFO 9 Server Path Traversal ... | 2026-04-24 | 2026-05-08 | Unknown | Samsung security updates, NVD URL | CWE-22, CWE-434 |
| CVE-2024-57728 | SimpleHelp | SimpleHelp | SimpleHelp Path Traversal Vulnerability | 2026-04-24 | 2026-05-08 | Unknown | SimpleHelp advisory, NVD URL | CWE-22 |
| CVE-2024-57726 | SimpleHelp | SimpleHelp | SimpleHelp Missing Authorization Vulnerabi... | 2026-04-24 | 2026-05-08 | Unknown | SimpleHelp advisory, NVD URL | CWE-862 |
| CVE-2026-39987 | Marimo | Marimo | Marimo Remote Code Execution Vulnerability | 2026-04-23 | 2026-05-07 | Unknown | GitHub advisory, NVD URL | CWE-306 |
| CVE-2026-33825 | Microsoft | Defender | Microsoft Defender Insufficient Granularit... | 2026-04-22 | 2026-05-06 | Unknown | MSRC, NVD URL | CWE-1220 |

### 4.4 `malicious_domains` (10건)
